...
//								List<String> obisDataList=instantParser.processInstantObisData(obisData);
								...
								if(file.getName().contains("INSTANT")) {
									...
									List<String> dataList=new DataProcessorAction().processObisDataList(obisHeaderList, obisDataList);
									status=instantParser.processInstantData(obisHeaderList, dataList,watchFolderPath);
								}else if(file.getName().contains("BILL")) {
									try {
										obisDataList=billingParser.processBillingObisData(obisData);
										billingParser.processBillingData(obisHeaderList, obisDataList, watchFolderPath,meterSerialNumber);
									}catch (Exception e) {
										e.printStackTrace();
									}
...
//										loadProfile1Parser.processLoadProfileObisData(obisHeaderList, obisDataList, watchFolderPath,meterSerialNumber);
									}catch (Exception e) {
										e.printStackTrace();
									}
